...
> 新增尺码替换与库存联动提示：顾客查看单品详情时，页面根据该单品在所选租期内的库存状态（可租/已租出/维修中）动态展示租赁按钮的可用性，若当前尺码在所选日期已租出，页面自动检测同款其他尺码是否有库存并提示"同款其他尺码可租"；顾客更换尺码或租期时，页面重新校验并更新租期日历中已被占用的日期标记（红色不可选），并同步刷新套装搭配卡中每件单品的可用状态；若套装中某单品因库存变化不可租，页面高亮该单品并推荐风格相近的替代单品（基于年代标签和品类自动匹配），替代后的搭配卡需重新评估尺码和风格一致性，确保顾客不因单品缺货放弃整套租赁。
